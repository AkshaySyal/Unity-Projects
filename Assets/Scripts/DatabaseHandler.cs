...
{
    private const string projectId = "first-17361"; // You can find this in your Firebase project settings
    ...
    public static void PostUser(User user, PostUserCallback callback)
    {
        if(user.score==0)
            RestClient.Put<User>($"{databaseURL}{"z"}{10000 - user.score}{user.name}.json?auth="+User.idToken, user).Then(response => { callback(); });
        if (user.score>90)
        RestClient.Put<User>($"{databaseURL}{0}{10000-user.score}{user.name}.json?auth="+User.idToken, user).Then(response => { callback(); });
        else
            RestClient.Put<User>($"{databaseURL}{10000 - user.score}{user.name}.json?auth="+User.idToken, user).Then(response => { callback(); });
    }
    ...
    {
        RestClient.Get<User>($"{databaseURL}{userId}.json?auth="+User.idToken).Then(user => { callback(user); });
    }
